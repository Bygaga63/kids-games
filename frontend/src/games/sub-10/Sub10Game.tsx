...
// все пары a−b, где a до 10 и ответ не меньше 1
function buildDeck(): Pair[] {
  const all: Pair[] = [];
  for (let a = 2; a <= 10; a++) for (let b = 1; b < a; b++) all.push({ a, b });
  return shuffle(all);
}

export default function Sub10Game() {
  ...
  return (
    <>
      <header className="bar">
        ...
        <span className="title">Вычитание до 10</span>
            ...
            <div key={taskKey} className="task pop">
              ...
              <div className="dots" aria-hidden="true">
                {Array.from({ length: cur.a }, (_, i) => (
                  <i
                    key={i}
                    className={i >= cur.a - cur.b ? 'out' : undefined}
                  />
                ))}
              </div>
            </div>
