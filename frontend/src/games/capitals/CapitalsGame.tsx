...
// 123 страны: флаг-эмодзи, название, столица (та же база, что в игре с флагами)
const DATA: Country[] = [
...
type Question = { current: Country; caps: string[] };
...
export default function CapitalsGame() {
  ...
  function next() {
    ...
    // 3 чужие столицы, все разные и не равные правильной
    let caps = [current.cap];
    const others = shuffle(DATA);
    for (let i = 0; i < others.length && caps.length < 4; i++) {
      if (caps.indexOf(others[i].cap) === -1) caps.push(others[i].cap);
    }
    caps = shuffle(caps);
    setQuestion({ current, caps });
    setPopKey((k) => k + 1);
  }

  function choose(cap: string) {
    if (locked || !question || finished) return;
    setLocked(true);
    setPicked(cap);
    const correct = cap === question.current.cap;
    if (correct) {
      sfx(true);
      addCoins(1);
      setScore((s) => s + 1);
      ...
      setToast({
        text:
          'Столица страны ' + question.current.n + ' — ' + question.current.cap,
        kind: 'bad',
      });
    }
  ...
  return (
    <>
      <header className="bar">
        ...
        <span className="title">Столицы стран</span>
      ...
      <main className="stage">
        ...
        {!finished && question && (
          ...
          >
            <div className="lead">Какая столица у этой страны?</div>
            <div key={popKey} className="country pop">
              <span className="flag">{question.current.f}</span>
              <span className="name">{question.current.n}</span>
            </div>
            <div className={'toast' + (toast ? ' ' + toast.kind : '')}>
              {toast?.text ?? ''}
            </div>
            <div className="opts">
              {question.caps.map((cap) => (
                <button
                  key={cap}
                  type="button"
                  disabled={locked}
                  className={
                    'opt' +
                    (locked && cap === question.current.cap ? ' correct' : '') +
                    (locked && picked === cap && cap !== question.current.cap
                      ? ' wrong'
                      : '')
                  }
                  onClick={() => choose(cap)}
